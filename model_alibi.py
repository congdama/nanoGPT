...
# get slopes: farther is smaller, closer is larger
def get_slopes(n):
    if math.log2(n).is_integer():
        return get_slopes_power_of_2(n)                   #In the paper, only train models that have 2^a heads for some a. This function has
    else:                                                      #some good properties that only occur when the input is a power of 2. To maintain that even
        closest_power_of_2 = 2**math.floor(math.log2(n))       #when the number of heads is not a power of 2, use this workaround. 
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.alibi = self.slopes.unsqueeze(1).unsqueeze(1) * torch.arange(self.maxpos).unsqueeze(0).unsqueeze(0).expand(self.attn_heads, -1, -1) # after the * is what constructs the diagonal matrix.
        self.alibi = self.alibi.view(self.attn_heads, 1, self.maxpos)
    ...
    def buffered_future_mask(self, tensor):
        ...
        if (
            ...
            self._future_mask = torch.triu(
                fill_with_neg_inf(torch.zeros([self.maxpos, self.maxpos])), 1
            )
